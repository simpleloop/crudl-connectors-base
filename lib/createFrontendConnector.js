...
function addLogs(mw) {
  ...
  return function (next) {
    const c = mw(next);
    return {
      create: (req) => { console.log(`Calling ${mwName}.create with ${req}`); return c.create(req); },
      read: (req) => { console.log(`Calling ${mwName}.read with ${req}`); return c.read(req); },
      update: (req) => { console.log(`Calling ${mwName}.update with ${req}`); return c.update(req); },
      delete: (req) => { console.log(`Calling ${mwName}.delete with ${req}`); return c.delete(req); },
    };
  };
}
